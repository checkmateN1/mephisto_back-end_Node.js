...
let PokerEngine = require('./pokerEngine');

// let initSetup = () => PokerEngine.InitSetup();
// let releaseSetup = (idSetup) => PokerEngine.ReleaseSetup(idSetup);
// let setPlayer = (idSetup, stack, position, adaptation) => PokerEngine.SetPlayer(idSetup, stack, position, adaptation);
...
class Session {
    constructor(sessionID) {
        ...
        this.setups[sessionID].timeout = setupTimeout;
...
const sessionsListener = (token, sessionID, request) => {
    if (token in sessions) {
        ...
        sessions[token].setups[sessionID] = new SessionSetup();
        return sessions[token].setups[sessionID].requestHandling(request);
    } else if (token in tokens) {
        sessions[token] = new Session(sessionID);
        sessions[token].timeout = sessionTimeout;
